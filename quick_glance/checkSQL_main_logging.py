...
requires 'checkSQL_db.py' and 'checkSQL_UserFu1n.py' to be in same working directory
"""
...
logging.basicConfig(filename='checkSQL.log',level=logging.INFO,format='%(levelname)s:%(message)s')
...
if __name__ == '__main__':
    ...
    dbs = Find_SQL_DB()
    if dbs.item_list:
        logging.info("Available databases: {}".format(dbs.item_list))
        while dbs.stop == False:
            ...
            logging.info("User chose database: {}".format(db_name))
            currdb = Explore_SQL(db_name)
            
            #Finds tables in the chosen Database
            tables = currdb.tables2list()
            logging.info("Available tables: {}".format(tables))
            if tables:
                while currdb.stop == False:
                    ...
                    logging.info("User chose table: {}".format(table_name))
                    ...
                    while currdf.stop == False:
                        ...
                        if 'no' in yes_no:
                            ...
                            logging.info("User examined variable: {}".format(dv_name))
    ...
    #If no .db files found, let User know
        ...
        logging.info("No databases in directory")
